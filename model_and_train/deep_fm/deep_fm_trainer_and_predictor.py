...
from model_and_train.deep_fm.deep_fm_model import DeepFM


class DeepFmSystem(BaseDeepLearningSystem):
    def __init__(self, train_ratio=0.9, dim=240, lr=0.0001, batch_size=256, epochs=5,
                 params_saved_path=os.path.join(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..')),
                                                'model_params', 'deep_fm_model_adamW_bce_dim=240_epoch=20.pth')):
        print('### 正在初始化DeepFM算法 ###')
        ...
        # 模型 + 加载参数
        self.dim = dim
        self.model = DeepFM(n_users, n_tags, dim).to(self.device)
    ...
    def __get_tags_by_book_id__(self, book_ids: torch.Tensor) -> torch.Tensor:
        """
        通过 book_id 加载 book_tags
        :param book_idS: 书本id 张量 shape = [batch_size]
    ...
    def update_train(self, newly_update_time: str) -> None:
        ...
        try:
            ...
            new_model = DeepFM(n_users, n_tags, self.dim).to(self.device)  # 创建(embedding)更高维度的模型
    ...
    def test_top_k(self, min_rating_num: int = 200, k: int = 100, threshold: float = 4.0) -> Tuple[float, float, float]:
        ...
        """
        return super(DeepFmSystem, self).test_top_k(min_rating_num, k, threshold)
